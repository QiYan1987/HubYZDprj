...
void CDoCircle::DrawCircle()
{
    ...
    /*memDC.MoveTo(dX0-EverystrcodX*10,dY0);							//qy 2018-06-15 数据text显示与数据不匹配
...
///////////////////////
   ...
   for (i=0;i<3600;i++)	
	...
	for( i=0;i<3600;i++)		
	{ 
		datas[i]=((CMainFrame*)AfxGetMainWnd())->m_OutIn*((dataafterfilter[i]-m_Middle))*ratio*0.2/(rect1.Height()*0.4/10)+RSLC; //绘制圆度打点时按比例显示
	...
	{ 
		...
		{  memDC.SelectObject(&pendeal);
	       memDC.SetPixel(drealx[i],drealy[i],RGB(0,255,0));//绿色曲线表示手动处理后的轮廓
		...
		else
		{
          ...
          memDC.SetPixel(drealx[i],drealy[i],RGB(255,0,0));//蓝色曲线表示实际轮廓
	      memDC.MoveTo(drealx[i], drealy[i]);
	      memDC.LineTo(drealx[i+m_Intstep], drealy[i+m_Intstep]);
		}
	}
...
void CDoCircle::MZCyuandu(double *Soudata)
{
	...
	for(i=0;i<GDN;i++)		
	{ 
		TsQ[i]=(Soudata[i]-2048)*10/(G1*G2*4096); //计算圆度误差转换位移公式	
	}
...
{
	  //MZCyuandu();//调用最小区域法评定圆度误差,里面包含读取文件的函数
	ReadDataFromFile();
	...
	double PV=m_PV[0];	    
	   ...
	   { 
		   ...
		   {  
			   memDC.SelectObject(&pendeal);
			   memDC.SetPixel(drealx[i]+rect1.left,drealy[i]+rect1.top,RGB(0,255,0));//蓝色曲线表示实际轮廓
		   ...
		   {
			   memDC.SelectObject(&penreal);
			   memDC.SetPixel(drealx[i]+rect1.left,drealy[i]+rect1.top,RGB(255,0,0));//蓝色曲线表示实际轮廓
			   memDC.MoveTo(drealx[i]+rect1.left, drealy[i]+rect1.top);
			   memDC.LineTo(drealx[i+m_Intstep]+rect1.left, drealy[i+m_Intstep]+rect1.top);
			   if(sqrt((drealx[i]-dX0)*(drealx[i]-dX0)+(drealy[i]-dY0)*(drealy[i]-dY0))<rout2)
				   rout2=sqrt((drealx[i]-dX0)*(drealx[i]-dX0)+(drealy[i]-dY0)*(drealy[i]-dY0));
	   ...
	   pencir.CreatePen(PS_SOLID,1*m_penScale, RGB(0,0,255));    
	   memDC.SelectObject(&pencir);
	   memDC.Ellipse(dX0-rout2+rect1.left,dY0-rout2+rect1.top,dX0+rout2+rect1.left,dY0+rout2+rect1.top);//绘制圆环外圆
	   memDC.Ellipse(dX0-rout3+rect1.left,dY0-rout3+rect1.top,dX0+rout3+rect1.left,dY0+rout3+rect1.top);//绘制圆环内圆
...
void CDoCircle::DrawCircleMCC()
{
	...
	double PV=m_PV[0];	   
	   ...
	   m_Intstep=5;//间隔几个点打点连线
		...
		{  
		   memDC.SelectObject(&pendeal);
		   memDC.SetPixel(drealx[i],drealy[i],RGB(0,255,0));//绿色曲线表示手动处理区域
		   memDC.MoveTo(drealx[i], drealy[i]);
		   memDC.LineTo(drealx[i+m_Intstep], drealy[i+m_Intstep]);	
		   if(sqrt((drealx[i]-dX0)*(drealx[i]-dX0)+(drealy[i]-dY0)*(drealy[i]-dY0))>rout2)
           rout2=sqrt((drealx[i]-dX0)*(drealx[i]-dX0)+(drealy[i]-dY0)*(drealy[i]-dY0));	   
		...
		for (i=3600-m_Intstep;i<3600; i+=m_Intstep)
		{
	       if (m_data[i].selected==1)
			{   
			    memDC.SelectObject(&pendeal);
			    memDC.SetPixel(drealx[i],drealy[i],RGB(0,255,0));//绿色曲线表示实际轮廓
		    	memDC.MoveTo(drealx[i], drealy[i]);
			    memDC.LineTo(drealx[0], drealy[0]);	
				if(sqrt((drealx[i]-dX0)*(drealx[i]-dX0)+(drealy[i]-dY0)*(drealy[i]-dY0))>rout2)
               rout2=sqrt((drealx[i]-dX0)*(drealx[i]-dX0)+(drealy[i]-dY0)*(drealy[i]-dY0));
...
void CDoCircle::DrawCircleMIC()
{
	  //读取文件并绘图
	  ReadDataFromFile();
  	  //调用最大内切圆圆心MICDX, MICDY,调用最大内切圆的最大值和最小值 DispMaxdirec,DispMindirec
...
void CDoCircle::Segmentationprocess()
{
	...
	memDC.MoveTo(fw1-fw2*1/2+rect1.left,fh+rect1.top); // 中线,横线
    ...
    memDC.LineTo(rect1.left+fw2*3/2+6,dtop+fh-fh2*7+fh2*0.4); 	
	memDC.TextOut(rect1.left+fw2,fh-fh1-50*m_penScale,"单位:um");	
    //绘制横坐标
	memDC.MoveTo(fw1-fw2*1/2+rect1.left,fh+fh2*7+rect1.top); // 中线,横线
...
{     
	...
	else
	{
		...
		pWnd1 = GetDlgItem(IDC_Deal_Circle);		// 获得对话框上的picture的窗口句柄
		pWnd1->GetClientRect(&rect1); 
		DrawrectWidth=rect1.Width();				// 获取绘制坐标的文本框
		...
		if(m_BOOLSegmentprocess==0  &&  FlagFrequency==0 )
		{
			...
			SubQKDataChuli(((CMainFrame*)AfxGetMainWnd())->m_fullPath,3600,
							 !((CMainFrame*)AfxGetMainWnd())->m_bAutoQK,fpdata, Resdata,m_IndexStart,m_IndexEnd); //自动去除缺口处理
			if (CirEvalue==0)
			{
//				FindQkMaxMin();
		...
		else  
		{
			...
			for (i=0;i<=m_Intendstart;i++)
			{
				if ((point.x) > fw2+m_smallavescale*i &&  (point.x) <fw2+(i+1)*m_smallavescale )
				{
				  ...
				  str.Format("%.4f",XRf4[i+2]*100);//幅值为
				  if (i<10-1)
				  {
					  str1.Format("%d",i+2);
					  str1=str1+"  ";
...
void CDoCircle::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	{	
		...
		CWnd *pwnd=GetDlgItem(IDC_Deal_Circle);//获取静态框的指针
		CDC *pDC=pwnd->GetDC();
		CRect rect_ctr;
		this->ClientToScreen(&point);  //转换为屏幕坐标
...
{	
	...
	if(m_BOOLSegmentprocess==0 && FlagFrequency==0)
	{
		...
		CWnd *pnd=GetDlgItem(IDC_Deal_Circle);                       //获取静态框的指针
		CDC *DC=pnd->GetDC();
...
void CDoCircle::DrawFilter (int  FilterMode ,double *pMem,double *drawdata)
{
    ...
    dw = 2 * 3.1415926 / 3600.0;// '每一次采样对应的角度间隔，弧度
	switch (FilterMode)
	 {
	case 0 ://'不滤波
	   for( i = 0;i<3600 ;i++)
...
void CDoCircle::OnFenDuanChuli() 
{
	...
	if(m_BOOLSegmentprocess==0 || FlagFrequency==1  ||FlagBogao==1)
	{
		...
		if (IDOK==dlg.DoModal())
		{
			...
			if (m_splitnumber<=0)
			{
				AfxMessageBox("输入的分段数必须大于等于0",MB_OK|MB_ICONERROR);
				return;
			}	
...
void CDoCircle::DrawGride1()
{
	...
	if (FlagBogao==0)
	{
		...
		memDC.FillSolidRect(dright,dtop+dy*9,gridRect.right-1,dy,RGB(232,232,232));//给指定行填充颜色
		...
		memDC.TextOut(mid,dtop+dy*9+meter," 测量参数 ");
		...
		memDC.TextOut(m1,dtop+dy*10+meter,"工艺要求");
		memDC.TextOut(m1,dtop+dy*11+meter,"圆度");
		memDC.TextOut(m1,dtop+dy*12+meter,"偏心");
		memDC.TextOut(m1,dtop+dy*13+meter,"偏心角度");   
		memDC.TextOut(m1,dtop+dy*14+meter,"放大倍数");
		memDC.TextOut(m1,dtop+dy*15+meter,"每格"); 

//		memDC.TextOut(m1 ,dtop+dy*16+meter,"片号");
//		memDC.TextOut(m1+dx/8 ,dtop+dy*16+meter," 最大值 ");
//		memDC.TextOut(m1+dx/8+dx/5,dtop+dy*16+meter," 最小值 ");
		
//		memDC.TextOut(m1+dx/8+dx/5+dx/4,dtop+dy*16+meter,"P+V ");
//		memDC.TextOut(m1+dx/8+dx/5+dx/5+dx/5,dtop+dy*16+meter,"邻片跳动量 ");  
	...
	//	memDC.TextOut(m3,dtop+dy*13+meter,"  °");
        memDC.TextOut(m3,dtop+dy*14+meter,"  倍");
...
/*		CString m_strLp,m_strmax,m_strmin,m_strLpjump,m_strpv;
		...
		{
	  	   memDC.TextOut(m2,dtop+dy*11+meter,m_strdRONT);//报表中显示最小二乘法计算结果
		   ...
		   if ( (FilterMode==8 || FilterMode==9) )
		   {
			   ...
			   memDC.TextOut(mid,dtop+dy*16+meter,"波纹度");
		...
		{	   
            ...
            memDC.TextOut(m2,dtop+dy*12+meter,m_strdEcc);//报表中显示最小区域法计算结果
//            memDC.TextOut(m2,dtop+dy*13+meter,m_strdEccPos);
		}
...
// 	pWnd1->GetClientRect(&rect1);             // 获取绘制坐标的文本框
	...
	rect1.top=tRect.top+25*yLogPixPerMM;//rect的上下左右都是表示的像素的个数
...
void CDoCircle::PrintSegmentationprocess()
{
	...
	memDC.MoveTo(fw1-fw2*1/2+rect1.left,fh+dtop); // 中线,横线
	...
	memDC.MoveTo(rect1.left+fw2*3/2,dtop+fh-fh2*7);  
    memDC.LineTo(rect1.left+fw2*3/2+6,dtop+fh-fh2*7+fh2*0.4); 	
	memDC.TextOut(rect1.left+fw2,rect1.top+fh-fh1-50*m_penScale,"单位:um");	
    //绘制横坐标
	memDC.MoveTo(fw1-fw2*1/2+rect1.left,dtop+fh+fh2*7); // 中线,横线
...
//  	m_AmplifyScale=dlg.m_AmpScale;	
  ...
  /*  memDC.MoveTo(dX0-EverystrcodX*10,dY0);						//qy 2018-06-15 数据与画盘匹配
    memDC.LineTo(dX0+EverystrcodX*10,dY0);//绘制x轴	
	memDC.TextOut(dX0+(rout+dright-dX0)/2,dY0-EverystrcodY/2,"0°");
	memDC.TextOut(rect1.left,dY0-EverystrcodY/2,"180°");
	memDC.TextOut(dX0+(rout+dright-dX0)/3,dY0-EverystrcodY*8,"45°");
   ...
   for (i=0;i<3600;i++)	
	...
	{ 
		...
		datas[i]=((CMainFrame*)AfxGetMainWnd())->m_OutIn*((dataafterfilter[i]-m_Middle))*ratio*0.2*5/(rect1.Height()*0.4/10)+RSLC; //绘制圆度打点时按比例显示 //绘制圆度打点时按比例显示
...
*/
	...
	double PV=m_PV[0];	   
	   ...
	   m_Intstep=5;//间隔几个点打点连线
	    ...
	    pencir.CreatePen(PS_SOLID,1*m_penScale, RGB(0,0,255));    
	    memDC.SelectObject(&pencir);
	    memDC.Ellipse(dX0-rout2,dY0-rout2,dX0+rout2,dY0+rout2);//绘制圆环外圆
	    double Rmincir=dX0/2-100*m_penScale;
	    pencir.DeleteObject();
...
//		  MZCyuandu();//调用最小区域法评定圆度误差,里面包含读取文件的函数
	...
	double PV=m_PV[0];	 
	   ...
	   memDC.SelectObject(&pencir);
	   memDC.Ellipse(dX0-rout2,dY0-rout2,dX0+rout2,dY0+rout2);//绘制圆环外圆
	   memDC.Ellipse(dX0-rout3,dY0-rout3,dX0+rout3,dY0+rout3);//绘制圆环内圆
	   double Rmincir=dX0/2-100*m_penScale;
	   pencir.DeleteObject(); 
...
/////////////////////////////////////////////////////////////////
	...
	int meter=rect1.Height()/70;
	if(1!=flag_bg){	
	memDC.TextOut(m1 ,dtop+meter,"片号");
	memDC.TextOut(m1+dx/8 ,dtop+meter," 最大值 ");
	memDC.TextOut(m1+dx/8+dx/5,dtop+meter," 最小值 ");
	
	memDC.TextOut(m1+dx/8+dx/5+dx/4,dtop+meter," P+V ");
	memDC.TextOut(m1+dx/8+dx/5+dx/5+dx/5,dtop+meter," 邻片跳动量 ");  
    }
	CString m_strLp,m_strmax,m_strmin,m_strLpjump,m_strpv;
	int i=0;
	for (i=1;i<=GQkNumber;i++)
