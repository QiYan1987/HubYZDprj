// Dlg_CZD.cpp : implementation file
//
...
CDlg_CZD::CDlg_CZD(CWnd* pParent /*=NULL*/)
	: CDialog(CDlg_CZD::IDD, pParent)
{
	//{{AFX_DATA_INIT(CDlg_CZD)
	m_name1 = _T("");
	m_high1 = 0.0f;
	m_PositionAway = 0.0f;
	m_bPre = FALSE;
...
void CDlg_CZD::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDlg_CZD)
	DDX_Control(pDX, IDC_LIST_HEIGHT, m_ctrListHeight);
	DDX_Text(pDX, IDC_EDIT_CZD_Name, m_name1);
	DDX_Text(pDX, IDC_EDIT_CZD_H, m_high1);
	...
	DDX_Text(pDX, IDC_EDIT_CZD_Num, m_num1);
...
BEGIN_MESSAGE_MAP(CDlg_CZD, CDialog)
	//{{AFX_MSG_MAP(CDlg_CZD)
	ON_BN_CLICKED(IDC_BTN_ADD, OnBtnAdd)
	ON_BN_CLICKED(IDC_BTN_DELETE, OnBtnDelete)
	ON_EN_CHANGE(IDC_EDIT_CZD_H, OnChangeEditCzdH)
	ON_BN_CLICKED(IDC_RADIO_SELECT, OnRadioSelect)
	ON_BN_CLICKED(IDC_RADIO2, OnRadio2)
	ON_EN_CHANGE(IDC_EDIT_CZD_Num, OnChangeEDITCZDNum)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CDlg_CZD message handlers

BOOL CDlg_CZD::OnInitDialog() 
{
...
//	strPath = strPath.Left(index+1);
	strPath+="canshu.ini";
	CString a;
	::GetPrivateProfileString("调整测量","m_fG11cursel_CZD","lisi",a.GetBuffer(100),100,strPath);
	m_fG11cursel=atoi(a);
	::GetPrivateProfileString("调整测量","m_bPre_CZD","lisi",a.GetBuffer(100),100,strPath);
	m_bPre=atoi(a);
	::GetPrivateProfileString("调整测量","m_PositionAway_CZD","lisi",a.GetBuffer(100),100,strPath);
	m_PositionAway=atof(a);
	::GetPrivateProfileString("调整测量","m_bBack0_CZD","lisi",a.GetBuffer(100),100,strPath);
	m_bBack0=atoi(a);
	::GetPrivateProfileString("调整测量","m_intSelect_CZD","lisi",a.GetBuffer(100),100,strPath);
	m_intSelect=atoi(a);
	::GetPrivateProfileString("调整测量","m_high1_CZD","lisi",a.GetBuffer(100),100,strPath);
	m_high1=atof(a);
	::GetPrivateProfileString("调整测量","m_num1_CZD","lisi",a.GetBuffer(100),100,strPath);
	m_num1=atoi(a);
	int i;
	for(i=0;i<m_num1;i++)
	{
		CString name;
		name.Format("m_height[%d]_CZD",i);
	...
	{
		(CEdit*)GetDlgItem(IDC_EDIT_CZD_H)->EnableWindow(TRUE);
		(CEdit*)GetDlgItem(IDC_EDIT_CZD_Num)->EnableWindow(TRUE);
	...
	{
		(CEdit*)GetDlgItem(IDC_EDIT_CZD_H)->EnableWindow(FALSE);
		(CEdit*)GetDlgItem(IDC_EDIT_CZD_Num)->EnableWindow(FALSE);
...
void CDlg_CZD::OnRadioSelect() //等间隔
{
...
void CDlg_CZD::OnChangeEditCzdH() 
{
	UpdateData(TRUE);	
	UpdataHeightList();
	
}
void CDlg_CZD::OnChangeEDITCZDNum() 
{
	UpdateData(TRUE);	
	UpdataHeightList();	
}
void CDlg_CZD::UpdataHeightList()
{
	m_ctrListHeight.DeleteAllItems();	
	CString str;
	char* data;
...
void CDlg_CZD::RefreshHeightList()
{
	//高度位置排序
	m_ctrListHeight.DeleteAllItems();	
	if (m_SumCount>1)
...
//	strPath = strPath.Left(index+1);
	...
	:: WritePrivateProfileString("调整测量","m_fG11cursel_CZD",A,strPath);	
	A.Format("%f",m_PositionAway);//写到配置文件中
    :: WritePrivateProfileString("调整测量","m_PositionAway_CZD",A,strPath);
	A.Format("%d",m_bPre);//写到配置文件中
    :: WritePrivateProfileString("调整测量","m_bPre_CZD",A,strPath);
	A.Format("%d",m_bBack0);//写到配置文件中
    :: WritePrivateProfileString("调整测量","m_bBack0_CZD",A,strPath);
	A.Format("%d",m_intSelect);//写到配置文件中
    :: WritePrivateProfileString("调整测量","m_intSelect_CZD",A,strPath);
	A.Format("%f",m_high1);//写到配置文件中
    :: WritePrivateProfileString("调整测量","m_high1_CZD",A,strPath);
	A.Format("%d",m_num1);//写到配置文件中
    :: WritePrivateProfileString("调整测量","m_num1_CZD",A,strPath);
	for(int i=0;i<m_num1;i++)
	{
