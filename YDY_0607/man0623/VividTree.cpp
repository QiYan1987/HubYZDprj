...
void VividTree::DrawItems( CDC *pDC )
{
	...
	do
	{
		...
		{
			if ( show_item == GetNextSiblingItem( m_hTreeSib ) )
			{
				rectSib = rc_item;
				m_hTreeSib = show_item;
				rectSib.left = rectClient.left;
				rectSib.right = rectClient.right;
				short sHeight;
				sHeight = GetItemHeight();
				rectSib.top -= sHeight/7;
				pDC->MoveTo( rectSib.left, rectSib.top );
				pDC->LineTo( rectSib.right, rectSib.top );
			}
